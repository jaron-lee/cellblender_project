...
\begin{enumerate} 
    ...
    \item<2->Spine head
    \item<3->Voltage-gated calcium channels (2)
    \item<4->Vesicles (2)
    \item<5->Glial cells
\end{enumerate}
    \end{column}
...
\begin{enumerate}
    ...
    \item<3->CaBS - a SNARE complex which has a single calcium ion bound
    \item<4->TAG - a SNARE complex which has two calcium ions bound
    \item<5->NT - a neurotransmitter molecule (represents glutamate)
    \item<6->LGIC - a neurotransmitter receptor residing on spine head (receptive to NT). Has open and closed states.
\end{enumerate}
}
...
\frame{
    ...
    \begin{itemize}
        ...
        \item Increase the scale of the model - more vesicles
